...
{ProdConsStats * stats = (ProdConsStats*) arg;
  while(get_cnt(&prod) < NUMBER_OF_MATRICES) {
	pthread_mutex_lock(&mutex);
	    ...
	    Matrix * mat = GenMatrixRandom();  //Get matrix
	    if(get_cnt(&prod) < NUMBER_OF_MATRICES) { //make sure get the updated current value
	    	put(mat);
	    ...
	    pthread_mutex_unlock(&mutex);
	}pthread_cond_broadcast(&empty_Condition); //weakup all sleeping threads, job done.
	return 0;
}
...
{ProdConsStats * stats = (ProdConsStats*) arg;
  while(get_cnt(&cons) < NUMBER_OF_MATRICES) {
		pthread_mutex_lock(&mutex);
	      //  check buffer to get data for m1, wait if empty_Condition
		while (counter == 0) {
		...
		increment_cnt(&cons); //increasing the consumer counter
		stats->matrixtotal += 1;
	...
	pthread_mutex_unlock(&mutex);
  	}pthread_cond_broadcast(&full_Condition); //weakup sleeping threads, job done
   return 0;
}
